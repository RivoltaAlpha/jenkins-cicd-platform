...
- **Gitleaks** - Secret scanning (in-repo, on every commit and again before deploy)
- **SonarQube** - Static code analysis (SAST)
- **OWASP Dependency Check** - Dependency vulnerability scanning (SCA)
- **Checkov** - Infrastructure-as-code scanning (Dockerfiles)
- **Trivy** - Container security scanning
- **Syft** - CycloneDX SBOM generation
- **Dependency-Track** - Continuous vulnerability tracking against the SBOM
...
| **Dependency-Track** | http://localhost:8092 | admin / admin (change on first login) |
...
**Purpose**: Catches hardcoded secrets (keys, tokens, passwords) in the repo.

**Access**: Reports archived as `gitleaks-report.json` (CI pass) and `gitleaks-final-report.json` (final pass before deploy) in Jenkins build artifacts.

**How to use**: Run a build, open the build's **Artifacts**, download the report. Any finding fails the build on test/prod.

### 6. Checkov (IaC Scanning)

**Purpose**: Scans the Dockerfiles for misconfigurations (e.g. running as root, missing HEALTHCHECK).

**Access**: Report archived as `checkov-report.json` in Jenkins build artifacts (test/prod builds only). Findings fail the build only on `prod`.

### 7. Syft + Dependency-Track (SBOM & Continuous Vulnerability Tracking)

**Purpose**: Syft generates a CycloneDX SBOM (`sbom.json`) from the built image; Dependency-Track ingests it and keeps re-scoring it against new CVEs even after the build finishes.

**Access**: http://localhost:8092 (frontend), http://localhost:8091 (API)

**Setup**: Requires a one-time API key — see the "Dependency-Track API Key" section in `ReadMe.md`. Without it, the SBOM is still generated and archived, just not uploaded.
...
- ✅ Gitleaks secret scanning (CI pass + final pre-deploy pass)
- ✅ OWASP dependency scanning
- ✅ Checkov IaC scanning (Dockerfiles)
- ✅ Trivy container scanning
- ✅ SonarQube SAST + quality gates
- ✅ Syft SBOM generation + Dependency-Track continuous tracking
